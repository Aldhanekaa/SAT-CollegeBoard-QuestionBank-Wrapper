...
import { Select } from "@/components/ui/select-calendar";
import {
  Select as UISelect,
  SelectContent,
  SelectItem,
...
const formatDateRange = (start: Date | null, end: Date, timezone: string) => {
  if (!start) {
    return `From oldest - ${formatInTimeZone(end, timezone, "EEE, MMM d")}`;
  }
...
export const Calendar = ({
  ...
  // Custom click outside handler that ignores select dropdown clicks
  useEffect(() => {
  ...
  const monthOptions = useMemo(() => {
    const months = [];
    for (let i = 0; i < 12; i++) {
      const monthDate = new Date(2023, i, 1); // Use a reference year
      months.push({
        value: i.toString(),
  ...
  // Generate year options (current year ± 10 years or limit to current year and past)
  const yearOptions = useMemo(() => {
    const currentYear = new Date().getFullYear();
    const years = [];

    if (limitToCurrentYear) {
      // Show from 10 years ago to current year (no future years)
      ...
      // Show range of years (current year ± 10 years)
  ...
  const handleYearChange = (yearValue: string) => {
    const newDate = new Date(parseInt(yearValue), currentDate.getMonth(), 1);
    setCurrentDate(newDate);
    setIsOpen(true);
  };
  ...
  const onApply = () => {
    ...
    if (showTimeInput) {
      parsedStartTime = parse(startTime || "", "HH:mm", new Date());
      parsedEndTime = parse(endTime || "", "HH:mm", new Date());
    }

    const hasDateErrors =
      (startDate.trim() && parsedStartDate?.toString() === "Invalid Date") ||
    ...
    if (hasDateErrors || hasTimeErrors) {
      ...
      if (showTimeInput) {
        ...
        parsedStart = parsedStartDate ? startOfDay(parsedStartDate) : null;
        parsedEnd = endOfDay(parsedEndDate);
      }

      onChange({
        start: parsedStart
          ? fromZonedTime(parsedStart, selectedTimezone)
          : null,
  ...
  // Check if current inputs match applied values
  useEffect(() => {
    const hasAppliedValues = appliedStart && appliedEnd;
    const hasAppliedTimeValues = showTimeInput
      ? appliedStartTime && appliedEndTime
      : true;

    if (hasAppliedValues && hasAppliedTimeValues) {
      const dateInputsChanged =
        startDate !== appliedStart || endDate !== appliedEnd;

      const timeInputsChanged =
        showTimeInput &&
        (startTime !== appliedStartTime || endTime !== appliedEndTime);

      if (dateInputsChanged || timeInputsChanged) {
        setIsApplied(false);
      }
    } else {
      setIsApplied(false);
          ...
          <div className={clsx(horizontalLayout && "flex gap-5")}>
              ...
              <div className="grid grid-cols-7 items-center gap-y-2">
                {daysArray.map((day) => {
                  ...
                  const isAllowedDate =
                    ...
                    (disableFuture ? day <= endOfDay(new Date()) : true);
            ...
            >
              <div className="flex flex-col gap-2">
                <div>
                  <div className="text-[13px] text-gray-900 capitalize flex items-center justify-between">
                    <span>Start</span>
                    <button
                      type="button"
                      onClick={() => setStartDate("")}
                      className="text-[11px] text-blue-600 hover:text-blue-800 underline"
                      title="Clear to use oldest date"
                    >
                      Clear
                    </button>
                  </div>
              ...
              <div className="flex flex-col gap-2">
                <div className="font-medium flex flex-col">
                  ...
                  >
                    {isApplied ? "Filter Applied" : "Apply Filter"}
                  </Button>
                </div>
