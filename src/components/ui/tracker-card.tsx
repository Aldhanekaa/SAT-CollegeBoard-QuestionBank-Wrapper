...
const TrackerCard = memo(function TrackerCard({
  ...
  const toggleQuestionsExpansion = useCallback(
    (taskId: string, subtaskId: string) => {
      const key = `${taskId}-${subtaskId}-questions`;
      console.log("Toggling questions expansion for:", key);
  ...
  // Toggle subtask status - memoized callback
          ...
          if (task.id === taskId) {
            const updatedSubtasks = task.subtasks.map((subtask) => {
              if (subtask.id === subtaskId) {
                // Cycle through statuses in order for subtasks
                const statuses = [
                  "completed",
                  "in-progress",
                  "pending",
  ...
  return (
    <div className="bg-background text-foreground h-full overflow-auto pb-2">
      <motion.div
              ...
              {tasks.map((task, index) => {
                ...
                return (
                  ...
                  >
                    {/* Task row */}
                    <motion.div
                      className="group flex items-start px-3 py-1.5 rounded-md"
                      whileHover={{
                    ...
                    >
                      <motion.div
                        className="mr-2 flex-shrink-0 cursor-pointer mt-1"
                        onClick={(e) => {
                          e.stopPropagation();
                                        ...
                                        {subtask.questions &&
                                          ...
                                          expandedQuestions[
                                            ...
                                            <div className="mt-0.5 mb-1">
                                              <span className="text-muted-foreground font-medium text-[10px] block mb-1">
                                                Questions (
                                                {subtask.questions.length}):
                                              </span>
