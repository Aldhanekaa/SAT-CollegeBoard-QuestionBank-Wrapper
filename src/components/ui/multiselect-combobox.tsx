 ...
 */
export interface GroupedOption extends BaseOption {
  group?: string;
  groupLabel?: string;
}
...
export const MultiSelectCombobox = <T extends BaseOption>({
  ...
  const groupedOptions = React.useMemo(() => {
    ...
    options.forEach((option) => {
      const groupKey = (option as any).group || "ungrouped";
      if (!groups[groupKey]) {
        groups[groupKey] = [];
      }
              ...
              Object.entries(groupedOptions).map(([groupKey, groupOptions]) => {
                const groupLabel =
                  groupOptions.length > 0
                    ? (groupOptions[0] as any).groupLabel || groupKey
                    : groupKey;
                return (
                  <CommandGroup
                    key={groupKey}
                    heading={
                      groupLabel !== "ungrouped" ? groupLabel : undefined
                    }
                  >
