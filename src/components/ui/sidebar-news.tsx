...
export function News({ articles }: { articles: NewsArticle[] }) {
  const [dismissedNews, setDismissedNews] = React.useState<string[]>(() => {
    // Load dismissed news IDs from localStorage on component mount
    if (typeof window !== "undefined") {
      try {
        const stored = localStorage.getItem("dismissedNewsIds");
        return stored ? JSON.parse(stored) : [];
      } catch (error) {
        console.warn("Failed to load dismissed news from localStorage:", error);
        return [];
      }
    }
    return [];
  ...
  // Update localStorage whenever dismissedNews changes
  React.useEffect(() => {
    if (typeof window !== "undefined") {
      try {
        localStorage.setItem("dismissedNewsIds", JSON.stringify(dismissedNews));
      } catch (error) {
        console.warn("Failed to save dismissed news to localStorage:", error);
      }
    }
  }, [dismissedNews]);
  ...
  return cards.length || showCompleted ? (
    ...
    >
      <div className="relative size-full">
        {cards.toReversed().map(({ id, href, title, summary, image }, idx) => (
          <div
            key={id}
            className={cn(
      ...
      <div className={cn(hideContent && "invisible")}>
        ...
        >
          <div className="flex items-center justify-between pt-3 text-xs">
            ...
            <Link
              href={href || "https://practicesat.vercel.app"}
              className="font-medium flex flex-row items-center gap-1 text-muted-foreground hover:text-foreground transition-colors duration-75 underline"
            >
              Try it out <ExternalLinkIcon size={12} />
            </Link>
          </div>
        </div>
      </div>
...
function AnimatedLogoOriginal(props: React.SVGProps<SVGSVGElement>) {
  return (
    <svg
      viewBox="0 0 48 21"
      fill="none"
...
function AnimatedLogo(props: React.SVGProps<SVGSVGElement>) {
  return (
    ...
    >
      ...
      <path
        d="M100.83 28.63L66.86 3.95c-7.25-5.26-17.07-5.26-24.35 0L8.54 28.63C1.29 33.89-1.76 43.23 1.01 51.77l12.98 39.93c2.77 8.53 10.72 14.3 19.7 14.3h41.97c8.98 0 16.93-5.76 19.7-14.3l12.98-39.93c2.77-8.53-.28-17.88-7.53-23.14Z"
        stroke="currentColor"
        strokeWidth="2"
        fill="none"
        strokeDasharray="300"
      ...
      {/* Inner star shape */}
      <path
        d="M64.81 63.13l-10.13 18.55-10.13-18.55-18.55-10.13 18.55-10.13 10.13-18.55 10.13 18.55 18.55 10.13-18.55 10.13Z"
        stroke="currentColor"
        strokeWidth="2"
        fill="none"
        strokeDasharray="150"
